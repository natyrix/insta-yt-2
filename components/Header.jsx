       ...
       <div className="flex items-center justify-end space-x-4">
          ...
          {session ?(<>
            ...
            <div className="absolute -top-1 -right-2 text-xs w-5 h-5 bg-red-500 rounded-full
              flex items-center justify-center animate-pulse font-semibold text-white">3</div>
          </div>
          <PlusCircleIcon onClick={()=>setOpen(true)} className='navBtn'/>
          <UserGroupIcon className='navBtn'/>
          <HeartIcon className='navBtn'/>

          <img src={session.user?.image} alt="" onClick={signOut} className='h-10 w-10 rounded-full cursor-pointer'/>
